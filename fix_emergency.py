...
# Criar script para corrigir o problema de uma vez

fix_script = '''#!/usr/bin/env python3
"""
Script de correção de emergência
Corrige skills/__init__.py e lucy_brain.py
"""
...
# 1. Recriar skills/__init__.py
init_content = """from .base_skill import BaseSkill
...
__all__ = ['BaseSkill', 'SpotifySkill', 'WeatherSkill', 'ReminderSkill', 'EmailSkill', 'SmartHomeSkill']
...
def get_skill(name, memory=None):
    skill_class = AVAILABLE_SKILLS.get(name.lower())
    if skill_class:
        return skill_class(memory)
    return None

def get_all_skills(memory=None):
    return [skill_class(memory) for skill_class in AVAILABLE_SKILLS.values()]
"""

# 2. Recriar lucy_brain.py (versão corrigida)
brain_content = """from datetime import datetime
import random
...
# Salvar arquivos
print("1. Criando skills/__init__.py...")
with open('skills/__init__.py', 'w', encoding='utf-8') as f:
    f.write(init_content)
print("   ✅ skills/__init__.py criado")

print("\\n2. Criando core/lucy_brain.py...")
with open('core/lucy_brain.py', 'w', encoding='utf-8') as f:
    f.write(brain_content)
print("   ✅ core/lucy_brain.py criado")
...
with open('/mnt/kimi/output/fix_emergency.py', 'w', encoding='utf-8') as f:
    f.write(fix_script)

print("✅ fix_emergency.py criado!")
print("\n🚨 Execute no terminal:")
print("   python fix_emergency.py")
print("\nIsso vai recriar os dois arquivos corrompidos!")
